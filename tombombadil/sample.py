...
def make_fn(pi_eq, log_pi, pimat, pimatinv, pimult, X, mask, include_invariant=True): # closure for defining fn (this change is mainly for making the unit testing easier, before it was a closure in run_sampler())
...
def _print_laplace_summary(params, se_natural):
    ...
    gtr_keys = ["alpha", "beta", "gamma", "delta", "epsilon", "theta", "omega"]
...
def save_params(output_stem: str, params: dict, mask: np.ndarray = None) -> None:
    ...
    scalar_keys = ["alpha", "beta", "gamma", "delta", "epsilon", "theta", "omega"]
